...
curl -v -X "POST" -H "Content-Type: application/yaml" -H "Accept: application/json" http://127.0.0.1:9080/graphdb/ref --data "node: { properties: { content: jo4neo makes it easy to provision a graph with data}, relations: [ { to: Node(name=Summer), type: author }, { to: Node(name=jo4neo), type: hasTag}  ] }"
