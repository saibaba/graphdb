...
def reference():
    return Node.ref(auth.get_tenant())
...
def dump_indexes():
    logging.info("DUMP Indexes...")
    for index, state in db.get_indexes():
        logging.info( "Kind: %s" % index.kind() )
        logging.info( "State: %s" % get_index_state_as_string(state))
        logging.info( "Is ancestor: %s" % index.has_ancestor() )
        for property_name, sort_direction in index.properties():
            logging.info( "  %s:%s" % (property_name, get_sort_direction_as_string(sort_direction)) )
